...
#Defines maxValue function using alpha beta pruning
def maxValue(arr1,a,b):
    global nodes
    val1 = 1
...
#Defines minValue function using alpha beta pruning
def minValue(arr1,a,b):
    global nodes
    val2 = -1
    if(terminalTest(arr1)!=0):
        return terminalTest(arr1)
...
#Defines minimaxDecision function which generates the move corresponding to a state
...
#Marks the corresponding slot which is evaluated using minimax on the GUI
def mark(arr,irr):
    x=0
    y=0
    for i in range (0,4):
